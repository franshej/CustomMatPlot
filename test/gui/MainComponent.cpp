...
#define PLOT_Y(Y) { \
((MainComponent*)thiz)->get_plot_holder()->push_back(std::make_unique<LinearPlot>());\
((MainComponent*)thiz)->get_plot_holder()->back()->updateYData(Y);\
thiz->addAndMakeVisible(((MainComponent*)thiz)->get_plot_holder()->back().get());}\
...
#define X_LIM(MIN, MAX) ((MainComponent*)thiz)->get_plot_holder()->back()->xLim(MIN, MAX);

#define Y_LIM(MIN, MAX) ((MainComponent*)thiz)->get_plot_holder()->back()->yLim(MIN, MAX);
...
#define TEST(f) static void f(juce::Component *thiz); \
static void __attribute__((constructor(101))) __construct##f(void){ \
	g_test_add(&f);\
} \
...
TEST(test_two_sine)
{
    std::vector<std::vector<float>> test_data_y = std::vector<std::vector<float>>(2, std::vector<float>(100));
		float i = 0;
		...
		std::vector<std::vector<float>> test_data_x = std::vector<std::vector<float>>(2, std::vector<float>(100));
