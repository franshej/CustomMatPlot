...
static void expectEqualVectors(const std::vector<float>& v1,
                               const std::vector<float>& v2,
                               const auto expectEquals) {
...
#define SECTION(TestClass, TestName)                       \
  class TestClass : public juce::UnitTest {                \
   public:                                                 \
    TestClass() : juce::UnitTest(#TestClass, #TestName) {} \
    void runTest() override;                               \
  };                                                       \
  static TestClass TestClass;                              \
  void TestClass::runTest()

#define TEST(TestName) beginTest(TestName);