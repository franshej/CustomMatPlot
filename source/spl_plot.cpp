...
static std::pair<float, float> findMinMaxValues(const std::vector<std::vector<float>>& xy_data)
{
    ...
    {
        const auto& current_max = *std::max_element(single_data.begin(), single_data.end());
        max_value               = current_max > max_value ? current_max : max_value;
        const auto& current_min = *std::min_element(single_data.begin(), single_data.end());
        min_value               = current_min < min_value ? current_min : min_value;
    }
    return {min_value, max_value};
}

static void setXLimGraphLines(const std::vector<std::unique_ptr<GraphLine>> &graph_lines,const float &min, const float &max){
	for (const auto &graph_line : graph_lines) {
		graph_line->xLim(min, max);
	}
}

static void setYLimGraphLines(const std::vector<std::unique_ptr<GraphLine>> &graph_lines,const float &min, const float &max){
	for (const auto &graph_line : graph_lines) {
...
void Plot::resized(){
	m_graph_area = juce::Rectangle<int>(0, 0, getWidth(), getHeight());
...
LinearPlot::LinearPlot(const int x, const int y, const int width, const int height){
...
void LinearPlot::updateXData(const std::vector<std::vector<float>> &x_data){
	if(!x_data.empty()){
		if(x_data.size() != m_graph_lines.size()){
			throw std::invalid_argument("Numbers of x_data vectors must be the same amount as y_data vectors. Make sure to set y_values first.");
		}
