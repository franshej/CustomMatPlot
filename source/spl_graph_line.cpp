...
GraphLine::GraphLine() : m_graph_colour(juce::Colour(std::rand() % 100 + 100, std::rand() % 100 + 100, std::rand() % 100 + 100)){}

void GraphLine::setPlotLimits()
{
    x_plot_limits.first  = getX();
    x_plot_limits.second = getX() + getWidth();
    y_plot_limits.first  = getY();
    y_plot_limits.second = getY() + getHeight();
}

bool GraphLine::isValueWihtinPlotLimits(const float& x, const float& y) const
{
...
void GraphLine::paint(juce::Graphics& g)
{
	...
	if (m_graph_points.size() > 1){
		...
		std::for_each(m_graph_points.begin() + 1, m_graph_points.end(), [&](const juce::Point<float> &point){graph_path.lineTo(point);});
		
		g.setColour(m_graph_colour);

		g.strokePath(graph_path,
...
void LinearGraphLine::calculateXData(){
	...
	if(nextXBlockReady && !m_x_data->empty()){
		if(m_graph_points.size() != m_x_data->size()){
			throw std::invalid_argument("x_values must be the same length as y_values.");
		}
...
void LinearGraphLine::calculateYData(){
	if(nextYBlockReady){
		...
		const auto offset_y = static_cast<float>(getHeight() + getY()) + (y_offset * y_scale);
